...
const imageTransform = async (
	...
	const imageInput: string = path.join(__dirname,'../','../','../','../','assets/','images/','input-files/',filename) + '.jpg';
	const imageOutputFolder: string = path.join(__dirname,'../','../','../','../','assets/','images/','output-files/');
	const imageOutput: string = path.join(__dirname,'../','../','../','../','assets/','images/','output-files/',filename) + `-${width}-${height}.jpg`;

	// create image output folder, if it doesn't exists, 
	if (!fs.existsSync(imageOutputFolder)) {
		await fsPromises.mkdir(imageOutputFolder);
	}

	try {
		// await for sharp to process the image, returns the image Output if successful
...
routes.get(
	...
	async (req: express.Request, res: express.Response): Promise<void> => {
		// validate if all 3 mandatory parameters are present and width & height are numeric
		if (validateMandatoryParams(req.query as unknown as QueryParam)) {
			...
			const outputImage: string = path.join(__dirname,'../','../','../','../','assets/','images/','output-files/',filename) + `-${width}-${height}.jpg`;
			...
			if (fs.existsSync(outputImage)) {
				...
				if (!String(imgProcessed).includes('Error')) {
					...
					res
						.status(404)
						.send(
							'No such image file exist, please verify the file name.'
						);
				}
			}
		} else {
