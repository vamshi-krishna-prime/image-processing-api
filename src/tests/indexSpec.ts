...
describe('Test image processing', () => {
	...
	const width = '300';
	const height = '300';
	const outputPath = path.join(__dirname,'../','../','assets/','images/','output-files/',filename) + `-${width}-${height}.jpg`;
...
describe('Test express related configutration', () => {
	...
	it('app is listening to port other than 0.', () => {
		expect((mainIndex.listen().address() as AddressInfo).port).toBeGreaterThan(
			0
		);
	});
});
